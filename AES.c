...
#include "Utils.h"
#define BLOCKSIZE 16
#define PADDING_MODE_ZEROS  //ZeroPadding
///#define PADDING_MODE_PKCS5_7    //Pkcs5Padding
...
int aesEncrypt(const uint8_t *key, uint32_t keyLen, uint8_t *pt, uint8_t *ct, uint16_t bufLen)
{
    ...
    uint32_t len = strlen(pt);

    #ifdef PADDING_MODE_ZEROS
    if (len % BLOCKSIZE != 0)
    {
        len = (BLOCKSIZE - len % BLOCKSIZE) + len;
    }
    else
    {
        len += BLOCKSIZE;
    ...
    if (len % BLOCKSIZE != 0)
    {
        int32_t len_num = BLOCKSIZE - len % BLOCKSIZE;
        int32_t i = 0;
        for (i; i < len_num; i++)
        {
            pt[len + i] = len_num;
    ...
    if (len % BLOCKSIZE || len > bufLen*2/3)
    {
    ...
    base64_encode(ct, ptrTmp, len);
    memcpy(ct, ptrTmp, strlen(ptrTmp));
    return 0;
}
...
int aesDecrypt(const uint8_t *key, uint32_t keyLen, uint8_t *ct, uint8_t *pt)
{
    ...
    ct = base64_decode(ct, &len);   //base64解码时获取AES密文长度为len
